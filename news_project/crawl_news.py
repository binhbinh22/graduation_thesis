...
def get_publish_date(article, article_soup):
    ...
    date_tag1 = article_soup.find('time')
    if date_tag1:
        return date_tag1.text.strip()
    return 'N/A'
def scrape_article(article_url, topic_name, author):
    try:
        ...
        # Check if extraction is enabled for the given author
        if Crawl.objects.filter(author=author.lower(), is_extract=True).exists():
            result = full_chain.invoke({"request": content})
            formatted_result = format_extraction_result(result.content)
            
            news_item.info_extrac = formatted_result
            news_item.save()
...
def find_article_links(soup, site):
    site_mappings = {
        ...
        'dantri':{'tag': 'h3','class': 'article-title'},
    }
...
# if __name__ == '__main__':
#     if len(sys.argv) != 5:
#         print("Usage: python crawl_news.py <base_url> <site> <topic> <author>")
#         sys.exit(1)
...
#     scrape_news_site(base_url, site.lower(), topic_name, author)
